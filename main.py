...
def sign(pi, message, key_pairs, pub_params):
    ...
    # print(ring_vec_ring_vec_mul_mod(big_h_2q, big_s_pi_2q, 2*Q))
    ...
    print("first", pi, first.coef[:4])
    print("second", pi, second.coef[:4])
    ...
    for i in range(pi + 1, pi + W):
        ...
        first = q_times_ci + ring_vec_ring_vec_mul_mod(big_a_i_2q, t[i], Q)
        print("first", i, first.coef[:4] % Q)

        second = q_times_ci + ring_vec_ring_vec_mul_mod(big_h_2q, t[i], Q)
        print("second", i, second.coef[:4] % Q)
        print("c", i, convert_bytes_to_poly(c[i]).coef[:4] % Q)
        c[i_plus_one] = h_one(big_l, big_h_2q, message, first, second)

    b = (-1) ** (np.random.randint(low=0, high=2))
    c_pi = convert_bytes_to_poly(c[pi])
    result = ring_vec_ring_mul_mod(big_s_pi_2q, c_pi, Q)
    for i in range(len(result)):
        result[i] += u[i]
    print(convert_bytes_to_poly(c[0]).coef[:4])
    t[pi] = result
...
def verify(signature, message, key_pairs, pub_params) -> bool:
    ...
    print(verified_c1.coef[:4] % Q)
    print(signed_c1.coef[:4] % Q)
